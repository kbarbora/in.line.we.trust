...
# def read_files(source_path, cpg_file, joern_path):
#     if type(DATASET) != type(pd.DataFrame()):
#         logging.error("Make sure to populate dataset first running main function")
#         return
#     commits_path = pathjoin(source_path, "commits")
#     cpg_path = pathjoin(source_path, cpg_file)
#     source_files = os.listdir(commits_path)
#     logging.info(f"Found {len(source_files)} files in the source directory to read")
#     toget = {}
#     max_len = 1  # max_len is needed to create the datasets separated by function names
#     for source in source_files:
#         source = source[0:-2]  # Delete '.c'
#         _splitted = source.split('--')
#         func = _splitted[0]
#         commit = _splitted[1]
#         if func in toget.keys():
#             toget[func].append(commit)
#             this_len = len(toget[func])
#             if this_len > max_len:
#                 max_len = this_len
#         else:
#             toget[func] = [commit, ]
...
# def create_cpg(func_commits: dict):
#     """
#     Get the vulnerable functions and create the cpg files to later extract the functions.
#     The main problem is that there are different definitons of a function (with same name)
#      across commits. If parse it through Joern, it will get the latest read.
#     """
#     _max = func_commits["max"]
#     files_stats = {}
#     # ds = []
#     func_commits.pop("max")
#     for i in range(_max):
#         _tmp = []
#         for f in func_commits.keys():
#             _this = func_commits[f]
#             if len(_this) == 0:
#                 continue
#             _tmp_name = f"{f}--{_this.pop()}.c"
#             _tmp.append(f"data/raw/{PROJECT}/commits/{_tmp_name}")
#         # ds.append(_tmp)
#         _tmp_path = pathjoin(TMP_DIR, PROJECT, str(i))
#         if not os.path.exists(_tmp_path):
#             os.makedirs(_tmp_path)
#             for t in _tmp:
#                 shutil.copy(t, f"{_tmp_path}")  # @TODO use symlinks instead of coping the files
#         _outfile = pathjoin(TMP_DIR, PROJECT, f"{i}.bin")
#         files_stats[_tmp_path] = (len(_tmp))
#         if os.path.exists(_outfile):
#             logging.warning(f"File {_outfile} exists. Skipping.")
#         else:
#             logging.info("Setting Java HEAP to a max of 8GB")
#             os.environ["JAVA_OPTS"] = "-Xms128M -Xmx8G"
#             logging.info(f"Creating CPG for source file located at {_tmp_path}")
#             subprocess.run([f"./{JOERN_PATH}joern-parse", _tmp_path, "--out", _outfile],
#                            stdout=subprocess.PIPE, text=True, check=True)
#             logging.info(f"CPG file create: {i}.bin")
#     return files_stats


# def parse_source(cpg_stats: dict, out_path: str):
#     """
#     Extract the function requested from the CPG and
#     write it to the raw project directory.
#     Extract data from only one CPG, will need to be run multiple
#     time to obtain all the functions.
#     """
#     for k in cpg_stats.keys():
#         _files = os.listdir(k)
#         # _files = [pathjoin(k, f) for f in _files] # add relative path to file (not needed)
#         if cpg_stats[k] != len(_files):
#             logging.error(f"Files count mismatched. On disk: {len(_files)} vs Stats: {cpg_stats[k]}. Continue.")
#         _func_toget = "%".join(_files)  # join each file in a str separated by % for joern processing
#         os.environ["JAVA_OPTS"] = "-Xms128M -Xmx10G"
#         subprocess.run(["./" + JOERN_PATH + "joern", "--script", pathjoin(JOERN_PATH, "dump_functions.sc"),
#                         "--params", f"cpgFile={k}.bin,functions_str={_func_toget},outFile={out_path}"])
#     return
...
def produce_df():
    ...
    the source code obtained from the scrapper in GitHub creates one dataframe for all projects
